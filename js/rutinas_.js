...
function conectayEjecuta__(aParametros,cPhp,aTabla){
    ...
    if (aTabla==null || aTabla==undefined){
        conexion1.open('PUT',"back/" + cPhp + "?aDatos="+aParametros,true);  // Prepara llamada al archivo PHP
    }else{
        aTabla = JSON.stringify(aTabla);
        conexion1.open('PUT',"back/" + cPhp + "?aDatos="+aParametros+"&aTabla="+aTabla,true); 
    }
...
function conectayEjecutaPost(aParametros, cPhp, aTabla) {
    ...
    if (aTabla == null || aTabla == undefined) {
        conexion1.open('POST', "back/" + cPhp, true); // Cambia PUT a POST y elimina los parámetros de la URL
    }else {
        aTabla = JSON.stringify(aTabla);
        conexion1.open('POST', "back/" + cPhp, true); // Cambia PUT a POST y elimina los parámetros de la URL ?????
    }
