	...
	switch ($vOpc) {
		case "validaLdap":
			...
			if ($validador["success"]==true){
				$_SESSION['SisCalError'] ="";
				header_remove('x-powered-by');
				header("location:../SisCal00_00.php");exit;
				return;
			}else{
				$_SESSION['SisCalError'] ="Credenciales Incorrectas o Inactivo";
				header_remove('x-powered-by');
		...
		if(getPermisos(pg_escape_string($vUsu))){ // Busca en la tabla de usuarios
			...
			if ($rol!=""){
				...
				if ( $salida=="1"){
					...
					$_SESSION['SisCalClave']      = $v_Datos[1];
					$_SESSION['SisCalApellidos']  = $v_Datos[3];
					$_SESSION['SisCalNombres']    = $v_Datos[4];
					$_SESSION['SisCalCurp']       = $v_Datos[6];
					$_SESSION['SisCalNC']		  = $v_Datos[10]; // Nombre completo(Empezando por apellidos)
					$_SESSION["SisCalPuesto"]     = $v_Datos[11];
					$_SESSION["SisCalEsquema"]	  = $rol;
					$_SESSION['SisCalTituloS']    = "Sistema de Saldos Calendarizados";
					$_SESSION["SisCalError"]	  = "";
