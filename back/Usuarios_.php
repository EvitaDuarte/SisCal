...
// * Sistema : Sistema de Calendarios              *
...
// * * * * * * * * * * * * * * * * * * * * * * * * *
	...
	if ( !isset($_SESSION['SisCalClave'])){
		header("Location: ../SisCal00_home.php");exit; return;
	}
	// _______________________________________
	$usrClave     = pg_escape_string($_SESSION['SisCalClave']); // Usuario que se logea
	//
	include_once("con_pg_SisCal_.php"); 		// Se incluye conexión a la Base de Datos
	include_once("rutinas_.php");
	require_once("pdo/Usuarios_c_.php");					// Clase Usuarios
	require_once("pdo/Esquemas_c_.php");					// Clase Usuarios
	...
	if ( isset($aDatos["idUsuario"])==false || isset($aDatos["nombre"])==false ){
		...
		$oUsuarios = new Usuarios( [ "idUsuario"=>$aDatos["idUsuario"], "Nombre" =>$aDatos["nombre"] , "idUnidad"  =>$aDatos["idUnidad"], 
									 "idEsquema"=>$aDatos["idEsquema"], "estatus"=>$aDatos["estatus"], "idusuarioa"=>$_SESSION['SisCalClave'] ] );
	}
